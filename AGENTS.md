...
8. 看情况是否可以通过更新`KNOWLEDGE.md`来帮助未来的开发，减少踩坑，提高token利用率等。
...
## Knowledge logging (must do)

- 本项目的长期知识记忆在：`KNOWLEDGE.md`
- **只要你发现任何未来会用到的知识（命令、坑点、约束、调试结论、配置、提高token利用率等），立刻把它追加到 `KNOWLEDGE.md` 最底部。**
- `KNOWLEDGE.md` 是 **append-only**：不要修改/删除旧记录；如果发现旧记录不对，就再追加一条“更正”。
