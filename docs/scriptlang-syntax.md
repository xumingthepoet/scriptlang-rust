...
- 同根 module 树内部，`<script>` 与 `<function>` 表达式的访问规则：
  - **短名访问**：只允许父模块访问子模块和自己内部元素（如 `navigation.get()`、`vals`）。
  - **长名访问**：必须使用全限定名，且目标必须显式导出（如 `m.vals` 需要父模块 `export=”const:vals”`）。
  - 子模块不能用短名访问父或兄弟元素，必须用全限定名（如 `m.vals`、`m.labels.x`）。
  - 根模块可直接调用子模块函数（如 `navigation.get()`），但子模块调用根模块函数需要全限定名。
