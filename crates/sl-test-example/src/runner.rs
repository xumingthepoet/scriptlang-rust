...
mod runner_tests {
    ...
    fn temp_dir(name: &str) -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("sl-test-example-runner-{}-{}", name, nanos))
    }
...
"#,
        ...
        assert!(matches!(
            wrong_kind_error,
            SlTestExampleError::ActionKindMismatch { .. }
        ));
...
"#,
    ...
    fn run_case_reports_unused_actions_and_engine_errors() {
        ...
        assert!(matches!(
            unused_error,
            SlTestExampleError::UnusedActions { .. }
        ));
...
"#,
        ...
        assert!(matches!(bad_choose_error, SlTestExampleError::Engine(_)));
    }
...
"#,
        ...
        assert!(matches!(error, SlTestExampleError::GuardExceeded { .. }));
    }
  ...
  "expectedEvents":[{"kind":"end"}]
        ...
        assert!(matches!(
            count_error,
            SlTestExampleError::EventCountMismatch { .. }
        ));
  ...
  "expectedEvents":[{"kind":"text","text":"Wrong"},{"kind":"end"}]
        ...
        assert!(matches!(
            value_error,
            SlTestExampleError::EventMismatch { .. }
        ));
    }
